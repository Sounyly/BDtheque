...
$nomError = $tomeError = $serieError = $genreError =  $typeError = $imageError = $nom = $tome = $serie = $genre = $type = $image = "";
	...
	if(!empty($_POST))
	{
        ...
        { 
            ...
            if($isImageUpdated)
            {
            	$statement = $db->prepare('UPDATE albums set name = ?, tome = ?, serie = ?, genre = ?, type = ?, images = ? WHERE id = ?');
            	$statement->execute(array($nom, $tome, $serie, $genre, $type, $image, $id));
            }
            else
            {
            	$statement = $db->prepare('UPDATE albums set name =?, tome = ?, serie = ?, genre = ?, type = ? WHERE id = ?');
        ...
        {
            $db = Database::connect();
            $statement = $db->prepare("SELECT * FROM albums where id = ?");
	...
	<body>
         ...
         <div class="container admin">
            <div class="row">
                ...
                <br>
                <form class="form" action="<?php echo 'update-bd.php?id='.$id;?>" role="form" method="post" enctype="multipart/form-data">
